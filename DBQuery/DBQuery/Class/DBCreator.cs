...
//			{
				...
				//for (int i = 0; i < fileListArray.Length; i++)
				{
					...
					using (StreamReader sr = new StreamReader(fileListArray[i]))
					{
						...
						{
							if (lineCount < DBRecord.featureHeaderNum)
							{
							...
							else
							{
								...
								if ((lineCount - DBRecord.featureHeaderNum + 1) % binMerge == 0)
								{
									doubleArray[DBRecord.featureHeaderNum - 1 + (lineCount - DBRecord.featureHeaderNum + 1) / binMerge] = binCount;
									binCount = 0;
								}
							}
			...
			for (int i = 0; i < fileListArray.Length; i++)
			{
				...
				{
					//ignore { Linearity(2), Sphericity(4) }
					if (j == 2 || j == 4)
						continue;
